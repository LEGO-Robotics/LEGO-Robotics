...
class IRBeaconDriver:
    def __init__(
            ...
            ir_sensor_port: str = INPUT_4,   # convention: https://sites.google.com/site/ev3devpython/learn_ev3_python/using-sensors
    ...
    def drive(self, speed: float = 100):
        ...
        elif self.ir_sensor.top_left(self.ir_beacon_channel) and self.ir_sensor.bottom_left(self.ir_beacon_channel):
            self.steer_driver.on(
                steering=-100,
                speed=speed)
    
        elif self.ir_sensor.top_right(self.ir_beacon_channel) and self.ir_sensor.bottom_right(self.ir_beacon_channel):
            self.steer_driver.on(
                steering=100,
                speed=speed)
