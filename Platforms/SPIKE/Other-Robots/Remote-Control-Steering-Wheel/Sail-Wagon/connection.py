...
class SpikeRCWheel(SpikePrimeStreamReader):
    def __init__(self, address):
        super().__init__(address)

    def right_paddle(self):
        return min( (self.device("B")[2]-381)/2.75, 100)

    def left_paddle(self):
        return min( (self.device("A")[0]-41)*2, 100)

    def left_thumb(self):
        return self.device("C")[2]

    def right_thumb(self):
        return self.device("D")[2]
